...
namespace NeuroNet
{
    public class Net
    {
        ...
        public int ContextSize => _layers[_layers.Length - 1].CountY;
        ...
        public Net(IActivation activation, int x, params int[] layer)
        {
            ...
            ContextWeights = new double[ContextSize, ContextSize];
            GenerateContextWeights(random, ContextSize);
        }
            ...
            if (recurrent)
            {
                ...
                for (int i = 0; i < _layers.Length; i++)
                {
                    if (i == 0)
                    {
                        ...
                        {
                            output[o] += Context.Select((Xn, n) => ContextWeights[o, n] * Xn).Sum();
                        }
                    }
        ...
        private void GenerateContextWeights(RandomNumbers random, int size)
        {
            var rnd = random;
            for (int l = 0; l < size; l++)
                for (var n = 0; n < size; n++)
                {
                    ContextWeights[l, n] = rnd.NextDouble(-1.0, 1.0);
                }
        }
        ...
        public void Corr(double[] inputs, double[] rightResult, double kLern = 0.85)
        {
            ...
            for (var k = _layers.Length - 1; k >= 0; k--)
            {
                ...
                // Изменяем веса контекста
                if (k == _layers.Length - 1)
                {
                    _layers[k].GetOut(inputs);

                    for (var x = 0; x < _layers[k].Outputs.Length; x++)
                    {
                        Context[x] = _layers[k].Outputs[x];
                    }
                }
